...
(function () {
  ...
  // hero wave bars (if present)
  const heroWaveContainer = document.getElementById('hero-wave');
  const heroWaveBars = [];
  if (heroWaveContainer && heroWaveContainer.children.length === 0) {
    for (let i=0;i<24;i++){ const b=document.createElement('span'); b.className='bar'; heroWaveContainer.appendChild(b); heroWaveBars.push(b);} 
  }
  ...
  if (audio) {
    ...
    function startViz() {
      ...
      function draw() {
        ...
        if (heroWaveBars.length){
          const n2 = heroWaveBars.length; const chunk2 = Math.floor(data.length / n2) || 1;
          for (let i=0;i<n2;i++){
            let s=0; const st=i*chunk2; const en=i==n2-1?data.length:st+chunk2; for(let j=st;j<en;j++) s+=data[j];
            const avg=s/(en-st); const sc=0.25 + (avg/255)*1.8; heroWaveBars[i].style.transform = `scaleY(${sc.toFixed(2)})`;
          }
        }
  ...
  // Embers removed for performance.
  // Split hero title into animated characters
  (function heroTyping(){
    const titles = $$('.hero-typing');
    titles.forEach(title => {
      if (title.dataset.splitted == '1') return;
